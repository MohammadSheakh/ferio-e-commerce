...
} from '../src/features/commerce-payments/queues/payment-recovery.queue';
...
const queue = new Queue<PaymentRecoveryJobData>(queueName, {
  connection,
  prefix: queuePrefix,
  defaultJobOptions: {
    attempts: 2,
  ...
  get: jest.fn(
    (key: string, fallback: string) =>
      ({
        ...
        PAYMENT_RECOVERY_EVERY_MINUTES: '60',
        PAYMENT_RECOVERY_BATCH_SIZE: '10',
      })[key] ?? fallback,
  ),
};
const payments = {
  ...
  expireAttempt: jest
    .fn()
    .mockRejectedValueOnce(new Error('intentional expiry retry'))
    .mockResolvedValue({ id: 'attempt-1', status: 'EXPIRED' }),
};
const recovery = new PaymentRecoveryQueue(
  queue,
  config as unknown as ConfigService,
  payments as unknown as CommercePaymentsService,
...
describe('Payment recovery BullMQ runtime smoke', () => {
  ...
  afterAll(async () => {
    await queue.removeJobScheduler(PAYMENT_RECOVERY_SCHEDULER_ID);
    await worker?.close();
    await events.close();
    await queue.obliterate({ force: true });
    await queue.close();
  ...
  it('schedules due attempts and retries expiry work', async () => {
    ...
    const sweep = (await queue.getWaiting()).find(
      (job) => job.name === PAYMENT_EXPIRY_SWEEP_JOB,
    );
    expect(sweep).toBeDefined();
    const sweepDone = sweep!.waitUntilFinished(events, 10000);
    worker = new Worker(queueName, (job) => processor.process(job), {
      connection,
      prefix: queuePrefix,
    });
    ...
    await expect(expiry!.waitUntilFinished(events, 10000)).resolves.toEqual(
