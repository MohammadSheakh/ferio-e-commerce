...
describe('TenantClosureService (PO-013)', () => {
  ...
  function build(closureStartedDaysAgo: number | null) {
    ...
    const platform = {
      client: {
        ...
        tenantDatabase: { update: jest.fn().mockResolvedValue({}) },
  ...
  it('initiating closure transitions to CLOSURE_PENDING and disables every domain', async () => {
    ...
    built.platform.client.organization.findUnique.mockResolvedValueOnce({
      id: 'org-1',
      status: 'ACTIVE',
      domains: [
    ...
    await built.service.initiateClosure('org-1', {
      actorId: 'po',
      reason: 'owner request',
    });

    const transitionCalls = built.organizations.transition.mock
      .calls as unknown as Array<[unknown, unknown, unknown]>;
    const transitionCall = transitionCalls.at(-1);
    expect(transitionCall?.[0]).toBe('org-1');
    expect(transitionCall?.[1]).toBe('CLOSURE_PENDING');
    expect(transitionCall?.[2]).toEqual(
  ...
  it('refuses finalization inside the 90-day recoverable window without override', async () => {
    ...
    expect(failure instanceof Error ? failure.message : '').toContain(
      'CLOSURE_RETENTION_PERIOD_ACTIVE',
    );
  ...
  it('allows finalization after the 90-day window, retiring registries only once', async () => {
    ...
    expect(built.platform.client.tenantDatabase.update).toHaveBeenCalledTimes(
      1,
    );
