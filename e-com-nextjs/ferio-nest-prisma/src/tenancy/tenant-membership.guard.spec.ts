...
describe('TenantMembershipGuard (MT-2 §5.3 / ADR-0004)', () => {
  ...
  function build(roster: MemberRowStub[]) {
    ...
    return { service, guard, findFirst: platformClient.organizationMember.findFirst };
  }

  type MemberRowStub = { id: string; organizationId: string; email: string; isActive: boolean; role: 'OWNER' | 'STAFF' };

  const contextFor = (user?: { email: string }, organizationId = 'org-A') =>
    ({
      switchToHttp: () => ({
        getRequest: () => ({ user, tenantOrganizationId: organizationId }),
      }),
    }) as never;

  // getTenantContext reads ALS; simulate via runWithTenantContext
  ...
  it('passes through in LEGACY mode without any roster lookup', async () => {
    ...
    const { guard, findFirst } = build([
      { id: 'm-1', organizationId: 'org-A', email: 'owner@acme.com', isActive: true, role: 'OWNER' },
    ]);
  ...
  it('accepts an authenticated member of the resolved organization', async () => {
    ...
    const request: any = { user: { email: 'Owner@Acme.com' } };
    await expect(
      runWithTenantContext(
        {
          organizationId: 'org-A',
  ...
  it('rejects a valid session that belongs to a DIFFERENT organization (cross-tenant replay)', async () => {
    ...
    const { guard, findFirst } = build([
      { id: 'm-2', organizationId: 'org-B', email: 'owner@other.com', isActive: true, role: 'OWNER' },
    ]);
    ...
    await expect(
      runWithTenantContext(
        ...
        () =>
          guard.canActivate({
            switchToHttp: () =>
              ({ getRequest: () => ({ user: { email: 'owner@other.com' } }) }) as never,
          } as never),
      ),
...
describe('TenantMembershipService roster caching', () => {
  it('caches lookups per org+email and invalidates on roster change', async () => {
    const row = { id: 'm-1', email: 'staff@acme.com', isActive: true, role: 'STAFF' as const };
    const platformClient = {
    ...
    await service.findActive('org-A', 'staff@acme.com');
    expect(platformClient.organizationMember.findFirst).toHaveBeenCalledTimes(1);

    service.invalidate('org-A', 'staff@acme.com');
    await service.findActive('org-A', 'staff@acme.com');
    expect(platformClient.organizationMember.findFirst).toHaveBeenCalledTimes(2);
  });
});
