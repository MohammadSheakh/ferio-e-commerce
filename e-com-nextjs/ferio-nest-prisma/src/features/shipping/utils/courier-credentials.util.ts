...
export function decryptCourierCredentials(
  ...
  if (raw.length <= IV_LENGTH + TAG_LENGTH) throw new Error('Encrypted courier credential payload is malformed');
  const decipher = createDecipheriv('aes-256-gcm', key(secret), raw.subarray(0, IV_LENGTH));
  ...
  if (!isCredentials(decoded)) throw new Error('Courier credentials are invalid');
  return decoded;
}
...
export function runWithCourierCredentials<T>(
  ...
  return credentials ? runtimeCredentials.run(credentials, callback) : callback();
}
    ...
    get(target, property, receiver) {
      if (property === 'get') {
        return (name: string, fallback?: unknown) => {
          ...
          return value ?? target.get(name, fallback as never);
        };
      }
      if (property === 'getOrThrow') {
        return (name: string) => {
          const value = runtimeCredentials.getStore()?.[name];
          return value ?? target.getOrThrow(name);
        };
      }
