...
export class ProductContentService {
  ...
  async submit(
    ...
    try {
      return await db.productYoutubeReview.create({
        data: {
          productId,
          submittedById: user.userId,
          youtubeUrl: dto.youtubeUrl,
          youtubeVideoId: this.videoId(dto.youtubeUrl),
      ...
      if (
        ...
        throw new ConflictException(
          'This video was already submitted for the product',
        );
      }
      throw error;
    }
  ...
  async adminReviews() {
    const db = await this.db();
    return db.productYoutubeReview.findMany({
      include: {
    ...
    return db.$transaction(async (tx) => {
      ...
      const updated = await tx.productYoutubeReview.update({
        ...
        data: {
          ...
          rejectionReason:
            targetStatus === 'REJECTED'
              ? (dto.rejectionReason?.trim() ?? review.rejectionReason)
              : null,
      ...
      await this.audit.record(
        {
          action: 'YOUTUBE_REVIEW_MODERATED',
          entityType: 'ProductYoutubeReview',
          entityId: id,
          actor,
  ...
  async deleteReview(id: string) {
    assertTenantCommerceWritable();
    const db = await this.db();
    return db.productYoutubeReview.delete({ where: { id } });
  }
  async banners(productId: string) {
    const db = await this.db();
    return db.productReviewBanner.findMany({
      where: { productId },
      orderBy: { sortOrder: 'asc' },
    });
  }
  async createBanner(productId: string, dto: CreateReviewBannerDto) {
    assertTenantCommerceWritable();
    const db = await this.db();
    return db.productReviewBanner.create({
      data: {
        productId,
  ...
  async updateBanner(id: string, dto: UpdateReviewBannerDto) {
    assertTenantCommerceWritable();
    const db = await this.db();
    return db.productReviewBanner.update({ where: { id }, data: dto });
  }
  async deleteBanner(id: string) {
    assertTenantCommerceWritable();
    const db = await this.db();
    return db.productReviewBanner.delete({ where: { id } });
  }
}
