    ...
    <form onSubmit={submit} className="mt-4 border-t border-line pt-4">
      ...
      <div className="mt-3 space-y-3">
        {entry.items.map((item) => (
          ...
          >
            ...
            <input
              name={`note-${item.id}`}
              aria-label={`Optional note for ${item.orderItem.productName}`}
              maxLength={500}
      ...
      <div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-5">
        <label className="text-[10px] text-ink2">
          Return reason
          <select name="reason" className={`mt-1 w-full ${fieldClass}`}>
          ...
          <article key={entry.id} className="py-5">
            ...
            {entry.status === "AWAITING_RECEIPT" ? (
              ...
              <div className="mt-3 text-[11px] text-ink2">
                <p>
                  {entry.reason ? formatEnum(entry.reason) : "Not classified"} ·{" "}
                  {entry.reasonNote}
                </p>
                {entry.items.map((item) => (
                  <p key={item.id}>
