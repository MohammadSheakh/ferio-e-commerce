...
export default function CategoriesPage() {
  ...
  const [viewingBrandsCategory, setViewingBrandsCategory] =
  ...
  async function saveCategory(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(
        form.id
          ? `/api/catalog/categories/${form.id}`
          ...
          <table className="w-full min-w-[720px] text-left">
            ...
            <tbody className="divide-y divide-line">
              {categories.map((category) => (
                <tr key={category.id} className="text-[13px]">
                  ...
                  <td className="px-5 py-3.5">
                    <span
                      className={`rounded-full px-2.5 py-0.5 text-[11px] font-medium ${category.isActive ? "bg-emerald-100 text-emerald-800" : "bg-surface text-ink2"}`}
                    >
                  ...
                  <td className="px-5 py-3.5 text-right">
                    <div className="flex justify-end items-center gap-3">
                      <button
                        ...
                        className="text-[12px] font-medium text-ink2 underline decoration-line underline-offset-4 hover:text-ink"
                      >
                        View brands
                      </button>
                      <button
      ...
      {viewingBrandsCategory && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby="category-brands-dialog-title"
            className="w-full max-w-lg space-y-4 rounded-card border border-line bg-white p-6"
          >
            <div className="flex items-center justify-between border-b border-line pb-3">
              <div>
                <h3
                  id="category-brands-dialog-title"
            ...
            {loadingCategoryBrands ? (
              <p className="py-8 text-center text-[13px] text-ink2">
                Loading brands...
              </p>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="ml-2 text-[11px] text-ink2 font-mono">
                          /{brand.slug}
                        </span>
                      </div>
                    </div>
              ...
              <div className="py-8 text-center text-[13px] text-ink2">
                No distinct saved brands found for products in this category
                yet.
              </div>
            )}
