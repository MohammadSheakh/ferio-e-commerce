export default function RequestedProductsLoading() {
  return (
    <div className="animate-pulse">
      ...
      <div className="flex flex-col gap-4 border-b border-line px-8 py-5 xl:flex-row xl:items-center xl:justify-between bg-white">
        <div>
          ...
          <div className="mt-1 h-3.5 w-52 rounded bg-slate-200" />
        </div>
      ...
      <div className="p-8 space-y-6">
        {/* Metric Summary Cards Skeleton (Pending, Collected, Contacted, Done) */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { label: "⏳ Pending", color: "border-amber-200 bg-amber-50/40" },
            { label: "📦 Collected", color: "border-blue-200 bg-blue-50/40" },
            { label: "📞 Contacted", color: "border-purple-200 bg-purple-50/40" },
            { label: "✅ Done", color: "border-emerald-200 bg-emerald-50/40" },
          ].map((c, i) => (
            <div key={i} className={`rounded-card border ${c.color} p-4 space-y-2`}>
              <p className="text-[11px] font-medium uppercase tracking-eyebrow text-slate-600">
                {c.label}
              </p>
              <div className="h-8 w-12 rounded bg-slate-300 font-semibold" />
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="h-9 w-64 rounded-full border border-line bg-paper" />
        </div>
          ...
          <div className="divide-y divide-line">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="px-5 py-4 flex items-center justify-between">
                <div className="h-4 w-28 rounded bg-slate-200 text-xs" />
                <div className="space-y-1 w-1/3">
                  <div className="h-4 w-full rounded bg-slate-300 font-medium" />
                ...
                <div className="h-4 w-24 rounded bg-slate-300 font-mono" />
                <div className="flex items-center gap-3">
                  <div className="h-7 w-28 rounded-full bg-slate-200" />
                  <div className="h-6 w-6 rounded-full bg-rose-50 border border-rose-100" />
                </div>
              </div>
            ))}
          </div>
