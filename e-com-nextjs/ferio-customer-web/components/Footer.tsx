  ...
  return (
    <footer className="mt-24 border-t border-line bg-paper">
      <div className="mx-auto grid max-w-6xl gap-12 px-6 py-16 md:grid-cols-3">
        <div>
          ...
          <p className="mt-3 max-w-xs text-[13px] leading-relaxed text-ink2">
            Current products, clear checkout totals, and order tracking in one
            place.
          </p>
          {store.legalName && (
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2.5 text-[13px] text-ink/80">
            ...
            <li>
              <Link href="/account/register" className="hover:text-ink">
                Create account
              </Link>
            </li>
            <li>
            ...
            {store.purchaseHistoryEnabled && (
              <li>
                <Link href="/purchase-history" className="hover:text-ink">
                  Recent verified purchases
                </Link>
              </li>
            )}
            <li>
              <Link href="/policies" className="hover:text-ink">
                Terms, privacy, and returns
              </Link>
            </li>
          </ul>
        </div>
        <div>
          ...
          <div className="mt-4 space-y-2.5 text-[13px] text-ink/80">
            ...
            {!store.supportPhone && !store.supportEmail && (
              <p className="text-ink2">Support contacts are being updated.</p>
            )}
            <p className="pt-2 text-[11px] text-ink2">
              Payment availability is confirmed during checkout.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-line">
        <div className="mx-auto flex max-w-6xl flex-col gap-2 px-6 py-6 text-[11px] text-ink2 sm:flex-row sm:items-center sm:justify-between">
          <span>{store.storeName}</span>
          <span>
            © 2026-{new Date().getFullYear() + 1} {store.storeName}. All rights
            reserved.
          </span>
        </div>
      </div>
