...
- [ ] **PARTIAL:** Add malware/content validation where required by upload type. (Buffered warranty uploads enforce size, allowlisted MIME types, and JPEG/PNG/WebP signature validation; direct presigned uploads enforce allowlisted MIME and signed size, and now require an authenticated tenant-bound `POST /admin/storage/finalize-put` inspection that verifies stored size, stored MIME, and magic bytes before the object is accepted. Malware scanning and provider-side quarantine remain required before this control is complete.)
